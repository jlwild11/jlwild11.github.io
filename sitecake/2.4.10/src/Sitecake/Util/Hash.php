...
class Hash
{
    ...
    public static function get($data, $path, $default = null)
    {
        ...
        switch (count($parts)) {
            ...
            case 3:
                return isset($data[$parts[0]][$parts[1]][$parts[2]]) ? $data[$parts[0]][$parts[1]][$parts[2]] :
                    $default;
            default:
        ...
        foreach ($tokens as $token) {
            ...
            if ($conditions) {
                ...
                foreach ($next as $item) {
                    if ((is_array($item) || $item instanceof ArrayAccess) && static::matches($item, $conditions)
                    ) {
                        $filter[] = $item;
                    }
                }
    ...
    /**
     * Exports data from flat array based on path in dotted notation
     *
     ...
     * @return array
     */
    public static function export($data, $path)
    {
        // Ensure there is no trailing dot
        $path = rtrim($path, '.');

        $baseExport = array_intersect_key(
            $data,
            array_flip(preg_grep('/^' . preg_quote($path) . '\./i', array_keys($data)))
        );

        ksort($baseExport);
        $baseExport = array_reverse($baseExport);

        $export = [];
        foreach ($baseExport as $key => $value) {
            $pathStripped = str_replace($path, '', $key);
            ...
            $newKey = ltrim($key, '.');
            if (substr_count($newKey, '.')) {
                $keyParts = explode('.', $newKey);
                $nextKey = array_shift($keyParts);
                if (isset($export[$nextKey]) && is_scalar($export[$nextKey])) {
                    $export[$nextKey] = [$export[$nextKey]] + self::export($data, $path . '.' . $nextKey);
                } else {
                    $export[$nextKey] = self::export($data, $path . '.' . $nextKey);
                }
            } else {
                $export[$newKey] = $value;
            }
     ...
     */
    protected static function tokenize($data, $separator = ',', $leftBound = '(', $rightBound = ')')
    {
        if (empty($data)) {
            return [];
        }
     ...
     */
    protected static function matches($data, $selector)
    {
        preg_match_all(
        ...
        foreach ($conditions as $cond) {
            ...
            } elseif (($op === '=' && $prop != $val) || ($op === '!=' && $prop == $val)
                || ($op === '>'
                    && $prop <= $val)
                || ($op === '<' && $prop >= $val)
                || ($op === '>=' && $prop < $val)
    ...
    protected static function simpleOp($op, $data, $path, $values = null)
    {
        ...
        foreach ($path as $i => $key) {
            if ((is_numeric($key) && (int)$key > 0 || $key === '0') && strpos($key, '0') !== 0
            ) {
                $key = (int)$key;
            }
            if ($op === 'insert') {
    ...
    public static function remove(array $data, $path)
    {
        ...
        $tokens = $noTokens ? explode('.', $path) : self::tokenize($path, '.', '[', ']');
     ...
     * `$valuePath` as path to read the values. If `$valuePath` is not specified, all values will be initialized
     ...
     * @see  sprintf()
     * @see  \Sitecake\Util\Hash::extract()
     */
     ...
     *                                 `static::doFilter()` Which strips out all non-zero empty values.
     *
     * @return array Filtered array
     */
    public static function filter(array $data, $callback = ['self', 'doFilter'])
    {
        foreach ($data as $k => $v) {
            if (is_array($v)) {
     ...
     * @return bool
     */
    protected static function doFilter($var)
    {
     ...
     * @return void
     */
    protected static function doMerge($stack, &$return)
    {
        while (!empty($stack)) {
            foreach ($stack as $curKey => &$curMerge) {
                foreach ($curMerge[0] as $key => &$val) {
                    $isArray = is_array($curMerge[1]);
                    if ($isArray && !empty($curMerge[1][$key]) && (array)$curMerge[1][$key] === $curMerge[1][$key]
                        && (array)$val === $val
                    ) {
     ...
     * to read the dimensions of the array.
     *
     ...
     * The function will read the extracted values as the first argument.
     *
     * ### Example
     *
    ...
    public static function normalize(array $data, $assoc = true)
    {
        ...
        if (!$numeric || $assoc) {
            ...
            for ($i = 0; $i < $count; $i++) {
                if (is_int($keys[$i])) {
                    $newList[$data[(string)$keys[$i]]] = null;
                } else {
                    $newList[(string)$keys[$i]] = $data[(string)$keys[$i]];
                }
            }
            $data = $newList;
        }
